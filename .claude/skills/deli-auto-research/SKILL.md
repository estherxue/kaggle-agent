...
description: >-
  Protocol framework for long-horizon autonomous agent runs (hours to days). Prescribes
  anti-stall, anti-cognitive-loop, and heartbeat-watchdog conventions: fresh-context iterations,
  file-persisted state, quantitative stall detection with forced structural pivots, and
  guardian/worker separation. Invoke when an autonomous or /loop task stalls, keeps trying similar
  directions with diminishing returns, waits for confirmation instead of executing, or needs
  orchestration across parallel fresh-context subagents (investigation / refutation / cross-domain
  analogy / verification). Source: Deli_AutoResearch, https://victorchen96.github.io/auto_research/
...
> Downloaded verbatim from https://victorchen96.github.io/auto_research/framework.html (SKILL.md).
> Installed in kaggle-agent because this repo runs long-horizon autonomous Kaggle work (the
> `Orchestrator`/`CompetitionState` loop in `src/kaggle_agent/`, and Claude Code `/loop` runs on
> active competitions). See the **kaggle-agent mapping** note at the bottom for how the framework's
> abstractions land on this codebase.
...
## kaggle-agent mapping (adopter notes)

How the framework's abstractions land on THIS repo — so the protocol is concrete here, not aspirational:

- **State files (§4)** already exist in spirit: `competitions/<slug>/state.json` (the resumable `CompetitionState`),
  `experiments/<id>/` (per-experiment records), and for the active MSTA red-team work the
  `competitions/multi-step-tool-attacks/PLAN.md` **results log IS `directions_tried.json`** (every
  vNN version + its LB score is a tried direction). Keep appending there, never overwrite.
- **Fresh session over resume (§3)** = the hard-won MSTA lesson: when reasoning plateaus, spawn
  **fresh-context** Agent-tool subagents (Pattern B) instead of grinding in the accumulated context.
  This is exactly what broke the "85.7 ceiling" false-conclusion — two fresh agents (extract-winners
  + refute-the-ceiling) saw what the loaded context could not.
- **Forced pivot on stall (§6)**: `stale_count>=2 → change a STRUCTURAL constraint`. MSTA v14–v19 were
  tactical tunes (multi-post, post+send, collapse templates) inside one frame and all missed; v20 was
  the structural pivot (per-MODEL template selection + measured ledger). Count consecutive
  no-improvement submissions and force a frame change, not another parameter tweak.
- **Ready means execute (§2.2)**: do NOT submit-then-idle-poll. After a submit, immediately build and
  validate the next principled variant (Pattern C: "start polling right after submit; auto-diagnose,
  fix, resubmit"). Idle waiting on a pending grade IS the §1.2 stall.
- **Guardian/worker separation (§7)**: a background poller / heartbeat may only check-liveness,
  restart, or nudge — it must not mutate the solution or submit on its own.
- **Verification (§8.D)**: keep the adversarial-verify habit (an independent subagent refutes each
  finding) — it caught the logloss-vs-balacc and dropped-NameError bugs in the S6E6 work and the
  false ceiling here.
