...
def main():
    ...
    ap.add_argument("--feat", choices=["logp", "logit"], default="logp")
    ap.add_argument("--calib", choices=["bias", "scale_shift", "both"], default="bias",
                    help="scale_shift DE is slow (~hours on full data); bias is the fast default")
    ...
    print(f"Nested-CV  models={models}  meta={args.meta}  feat={args.feat}  calib={args.calib}  feats={X.shape[1]}")
    if args.calib in ("bias", "both"):
        ba_bias = nested_cv(X, y, args.meta, fit_additive_bias, args.seed)
        print(f"  nested-CV BA (additive bias)   : {ba_bias:.5f}")
    if args.calib in ("scale_shift", "both"):
