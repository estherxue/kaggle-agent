"""s6e6 xgbv5: FAITHFUL port of cdeotte-style "PS-S6E6 RAPIDS XGBoost GPU Model v5"
(reproduces local experiment EXP3-110; reported public LB ~0.96801).
...
Outputs to /kaggle/working/:
  oof_xgbv5.npy   (577347, 3) float32 OOF probabilities, train-CSV row order, [GALAXY,QSO,STAR]
  test_xgbv5.npy  (247435, 3) float32 fold-averaged test probabilities, sample_submission order
  results.txt     per-fold + overall OOF balanced accuracy + per-class recall
...
OOF_PATH = WORK / 'oof_xgbv5.npy'
PRED_PATH = WORK / 'test_xgbv5.npy'
...
# ---------------------------------------------------------------------------
# Train XGBoost (verbatim hyperparameters from source; device='cuda')
# ---------------------------------------------------------------------------
...
def class_weights(y_series):
    y_np = np.asarray(y_series, dtype=np.int32)
    counts = np.bincount(y_np, minlength=len(CLASSES)).astype(np.float32)
    weights_per_class = np.float32(len(y_np)) / (np.float32(len(CLASSES)) * np.maximum(counts, np.float32(1.0)))
    weights = weights_per_class[y_np]
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(y_cpu), dtype=np.int8), y_cpu), start=1):
    ...
    sample_weight = class_weights(y_tr) if USE_CLASS_WEIGHTS else None
    valid_weight = class_weights(y_va) if USE_CLASS_WEIGHTS else None
    ...
    del model, X_tr, X_va, X_te, X_tr_np, X_va_np, X_te_np, y_tr, y_va, y_tr_np, y_va_np
...
# Per-class recall (GALAXY/QSO/STAR) on the OOF predictions
...
# ---------------------------------------------------------------------------
# Feature importance (top 25) -> results.txt
# ---------------------------------------------------------------------------
top_importance = (
...
emit('\nTop 25 features by mean gain:')
emit(top_importance.head(25).to_string(index=False))


# ---------------------------------------------------------------------------
# Save artifacts (normalize + clip for stacking)
# ---------------------------------------------------------------------------
